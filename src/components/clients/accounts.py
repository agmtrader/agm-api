...
def get_all_watchlists() -> dict:
    """Get all IBKR watchlists available for the active SSO session."""
    return ibkr_web_api.get_all_watchlists()

@handle_exception
def get_watchlist_information(watchlist_id: str) -> dict:
    """Get one IBKR watchlist by id for the active SSO session."""
    return ibkr_web_api.get_watchlist_information(watchlist_id=watchlist_id)

@handle_exception
def get_market_data_snapshot(conids: str) -> dict:
    """Get IBKR market snapshot data for one or more conids."""
    return ibkr_web_api.get_market_data_snapshot(conids=conids)

@handle_exception
def get_market_scanner_params() -> dict:
    """Get IBKR market scanner parameters."""
    return ibkr_web_api.get_market_scanner_params()

@handle_exception
def run_market_scanner(instrument: str, scan_type: str, location: str, filters: list = None) -> dict:
    """Run the IBKR market scanner for the active SSO session."""
    return ibkr_web_api.run_market_scanner(
...
@handle_exception
def get_historical_market_data(conid: str, period: str, bar: str) -> dict:
    """Get IBKR historical market data for the active SSO session."""
    return ibkr_web_api.get_historical_market_data(conid=conid, period=period, bar=bar)

@handle_exception
def get_securities_by_symbol(symbol: str, sec_type: str) -> dict:
    """Search securities by symbol and type for the active SSO session."""
    return ibkr_web_api.get_securities_by_symbol(symbol=symbol, sec_type=sec_type)

@handle_exception
def get_security_info(issuer_id: str, sec_type: str) -> dict:
    """Get security info by issuer id and type for the active SSO session."""
    return ibkr_web_api.get_security_info(issuer_id=issuer_id, sec_type=sec_type)

@handle_exception
def get_all_conids_from_exchange(exchange: str) -> dict:
    """Get all conids for one exchange from IBKR."""
    return ibkr_web_api.get_all_conids_from_exchange(exchange=exchange)

@handle_exception
def get_contract_info(conid: int) -> dict:
    """Get IBKR contract metadata for one conid."""
    return ibkr_web_api.get_contract_info(conid=conid)

@handle_exception
def place_order(account_id: str, orders: list) -> dict:
    """Place one or more IBKR orders for the active SSO session."""
    return ibkr_web_api.place_order(account_id=account_id, orders=orders)

@handle_exception
def reply_to_order(reply_id: str, confirmed: bool = True) -> dict:
    """Reply to an IBKR order confirmation prompt."""
    return ibkr_web_api.reply_to_order(reply_id=reply_id, confirmed=confirmed)

@handle_exception
def cancel_order(account_id: str, order_id: str) -> dict:
    """Cancel an IBKR order for the active SSO session."""
    return ibkr_web_api.cancel_order(account_id=account_id, order_id=order_id)

@handle_exception
def get_open_orders() -> dict:
    """Get all open IBKR orders for the active SSO session."""
    return ibkr_web_api.get_open_orders()
